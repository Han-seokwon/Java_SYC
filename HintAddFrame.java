		...
		public void addHintViewButtonpanel(Problem problem, User user) { // 힌트보기프레임버튼패널
			...
			hintRegistbtn.addActionListener (new ActionListener() { //익명클래스 힌트작성버튼 리스너
				public void actionPerformed(ActionEvent e) {
					...
					// 힌트 리스트에 추가 리스너
					String hintContent = hintwritefield.getText();
					String hintstep = "Step " + String.valueOf(step);
					
					// 힌트 리스트에 작성한 힌트내용 추가
					new LearningMaterialsHintAdd(hintstep, user, hintContent, problem);
			...
			// 뒤로가기 버튼
			DesignedButton hintCancelBtn = new DesignedButton("뒤로가기", 150, 40, COLOR.AQUA_ISLAND);
